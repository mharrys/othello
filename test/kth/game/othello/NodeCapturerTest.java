...
public class NodeCapturerTest {
	...
	public void nodesToCaptureInDirection() {
		...
		Assert.assertEquals(1, nc.nodesToCaptureInDirection(nodes, player2, from, direction).size());
		
		Mockito.when(direction.getXCoordinate()).thenReturn(4);
		Mockito.when(direction.getYCoordinate()).thenReturn(3);
		Assert.assertEquals(0, nc.nodesToCaptureInDirection(nodes, player2, from, direction).size());
